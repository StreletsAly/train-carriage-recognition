...
# Определение класса MORN (Morphable Operation Rectification Network), который наследуется от nn.Module
class MORN(nn.Module):
    # Конструктор класса
    def __init__(self, nc, targetH, targetW, inputDataType='torch.cuda.FloatTensor', maxBatch=256, CUDA=True):
        super(MORN, self).__init__() # Вызов конструктора родительского класса nn.Module
        self.targetH = targetH # Целевая высота выходного изображения
        self.targetW = targetW # Целевая ширина выходного изображения
        self.inputDataType = inputDataType # Тип входных данных (cuda или cpu)
        self.maxBatch = maxBatch # Максимальный размер батча
        self.cuda = CUDA # Флаг использования GPU

        # Определение сверточной сети (CNN) для вычисления смещений
        self.cnn = nn.Sequential(
            nn.MaxPool2d(2, 2),  # Максимальное объединение с ядром 2x2 и шагом 2
            nn.Conv2d(nc, 64, 3, 1, 1), nn.BatchNorm2d(64), nn.ReLU(True), nn.MaxPool2d(2, 2), # Свертка -> нормализация -> активация -> пулинг
            ...
            nn.Conv2d(16, 1, 3, 1, 1), nn.BatchNorm2d(1) # Свертка, уменьшающая количество каналов до 1
            )

        # Определение пула для последующего объединения
        self.pool = nn.MaxPool2d(2, 1)

        # Создание сетки координат для морфинга
        h_list = np.arange(self.targetH)*2./(self.targetH-1)-1 # Координаты по высоте
        w_list = np.arange(self.targetW)*2./(self.targetW-1)-1 # Координаты по ширине

        # Генерация сетки с помощью meshgrid
        grid = np.meshgrid(
                w_list, 
                h_list, 
                indexing='ij'
            )
        grid = np.stack(grid, axis=-1) # Объединяем сетку по последней оси
        grid = np.transpose(grid, (1, 0, 2)) # Транспонирование сетки
        grid = np.expand_dims(grid, 0) # Добавляем размерность batch
        grid = np.tile(grid, [maxBatch, 1, 1, 1]) # Копируем сетку для всего батча
        grid = torch.from_numpy(grid).type(self.inputDataType) # Преобразуем в тензор PyTorch
        if self.cuda:
            grid = grid.cuda() # Переносим сетку на GPU, если доступно

        # Сохранение сетки в Variable
        self.grid = Variable(grid, requires_grad=False)
        self.grid_x = self.grid[:, :, :, 0].unsqueeze(3) # Координаты по ширине
        self.grid_y = self.grid[:, :, :, 1].unsqueeze(3) # Координаты по высоте

    # Прямой проход (forward) через сеть
    def forward(self, x, test, enhance=1, debug=False):

        # Условие для выполнения upsample (увеличение изображения), если test=False и вероятность > 0.5
        if not test and np.random.random() > 0.5:
            return nn.functional.upsample(x, size=(self.targetH, self.targetW), mode='bilinear')
        if not test:
            enhance = 0

        # Убедимся, что размер батча не превышает максимальный
        assert x.size(0) <= self.maxBatch
        assert x.data.type() == self.inputDataType # Проверка типа входных данных

        # Получение сетки для текущего батча
        grid = self.grid[:x.size(0)]
        grid_x = self.grid_x[:x.size(0)]
        grid_y = self.grid_y[:x.size(0)]
        # Уменьшение изображения до заданного размера
        x_small = nn.functional.upsample(x, size=(self.targetH, self.targetW), mode='bilinear')

        # Пропуск через CNN для вычисления смещений
        offsets = self.cnn(x_small)
        offsets_posi = nn.functional.relu(offsets, inplace=False) # Применение ReLU к положительным значениям
        offsets_nega = nn.functional.relu(-offsets, inplace=False) # Применение ReLU к отрицательным значениям
        offsets_pool = self.pool(offsets_posi) - self.pool(offsets_nega) # Вычисление разности пулов

        # Интерполяция сетки с учетом смещений
        offsets_grid = nn.functional.grid_sample(offsets_pool, grid)
        offsets_grid = offsets_grid.permute(0, 2, 3, 1).contiguous()
        offsets_x = torch.cat([grid_x, grid_y + offsets_grid], 3) # Добавление смещений к сетке
        x_rectified = nn.functional.grid_sample(x, offsets_x) # Применение сетки к входному изображению

        # Итеративное улучшение выравнивания
        ...
        if debug:
            ...
            return x_rectified, img_processed
